...
def run_demo():
    ...
    err_sum = 0.0  # Sum of errors for integral term
    err_old = 0.0  # Previous error for derivative term
    dt = 0.1       # Time step
    ...
    while not rospy.is_shutdown():
        ...
        # PID control
        err = distance  # Error: distance to the flag
        err_sum += err * dt  # Integral of error
        d_err = (err - err_old) / dt  # Derivative of error
        err_old = err

        # Calculate velocity using PID formula
        velocity = Kp * err + Ki * err_sum + Kd * d_err

        # Limit the velocity to a maximum safe value
        velocity = robot.constraint(velocity, 0, 2)

        # If the robot is very close to the flag, stop
        if distance <= 3:
            velocity = 0

        # Publish the desired speed
        robot.set_speed_angle(velocity, 0)
        rospy.sleep(dt)
