...
class RobotController:
    ...
    def __init__(self, robot_name):
        ...
        self.distance_init = self.robot.getDistanceToFlag()
        self.pid_controller = PIDController(1.0, 0.1, 1.0)

    def run_demo(self):
        """Main loop"""
        rospy.loginfo(f"Robot: {self.robot_name} is starting...")

        while not rospy.is_shutdown():
            # Get sonar distance
            sonar_distance = self.robot.get_sonar()

            # Get distance to flag
            flag_distance = self.robot.getDistanceToFlag()
            print("flag_distance = ", flag_distance)

            # Use PID controller to control speed and angle
            speed = self.pid_controller.update(flag_distance)
            angle = self.pid_controller.update(self.robot.yaw)

            # Check for obstacles
            if sonar_distance < 0.5:

                # Stop robot
                self.robot.set_speed_angle(0, 0)

                # Turn robot to avoid obstacle
                angle = self.pid_controller.update(0.5)

            else:
                # check if the robot is moving away from flag
                if flag_distance > self.distance_init:
                    angle = 0.03
                    ...
                    angle = -0.092
            self.robot.set_speed_angle(speed, angle)
            rospy.sleep(0.5)
